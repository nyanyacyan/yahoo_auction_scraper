...
class WriteGssFlow:
    ...
    def format_image_formula(self, url_or_formula):
        """URL なら =IMAGE("...",4,80,80) を作成。既に数式ならそのまま返す。"""
        if not url_or_formula:
            return ""
        s = str(url_or_formula).strip()
        if s.startswith("="):     # 例: '=IMAGE("...")' はそのまま
            return s
        return f'=IMAGE("{s}", 4, 80, 80)'
    ...
    def build_write_list(self, records):
        write_list = []
        for rec in records:
            # 画像は 'image_url' 優先、無ければ 'image'（=IMAGE式の可能性あり）を利用
            img_src = rec.get("image_url") or rec.get("image") or ""

            row = [
                self._safe_sheet_text(rec["date"]),     # ← 二重 ' 防止
                rec["title"],
                rec["price"],
                rec["ct"],
                rec["1ct_price"],
                self.format_image_formula(img_src),      # ← 二重 =IMAGE 防止
            ]
